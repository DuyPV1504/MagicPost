...
const PackageStatistic = (smallestYear: any) => {
    ...
    const curPoint = localStorage.getItem("point_reference");
    ...
    const { data: curPackageFail, isLoading: isLoading } = useGetList('delivery',
        {
            filter: filter,
            pagination: {
                page: 1,
                        ...
                        (
                            <>
                                <Stack direction="row" spacing={2}>
                                    ...
                                    <FailCard value={getTotal(curPackageFail)} />
                                </Stack>
                                <Typography variant="h6" gutterBottom>
                                    {`Thống kê hàng nhận thành công tại ${curPoint} ${month !== 0 ? month.toString() + '/' + year.toString() : year.toString()}`}
                                </Typography>
                                    ...
                                    <ResponsiveContainer>
                                        <AreaChart data={month !== 0 ? getTotalPerDayRecv(month, year, curPackageReceive) : getTotalPerMonthRecv(year, curPackageReceive)}>
                                            ...
                                            <Tooltip
                                                ...
                                                formatter={(value: any) =>
                                                    value.toString() + " hàng nhận thành công"
                                                }
                                ...
                                <Typography variant="h6" gutterBottom>
                                    {`Thống kê hàng nhận không thành công tại ${curPoint} ${month !== 0 ? month.toString() + '/' + year.toString() : year.toString()}`}
                                </Typography>
                                <div style={{ width: '100%', height: 300 }}>
                                    <ResponsiveContainer>
